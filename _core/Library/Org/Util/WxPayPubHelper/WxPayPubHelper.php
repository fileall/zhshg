...
/*		$file  = 'log.txt';//要写入文件的文件名（可以是任意文件名），如果文件不存在，将会创建一个		
		if($f  = file_put_contents($file, $xml,FILE_APPEND)){// 这个函数支持版本(PHP 5) 
			echo "写入成功。<br />";
		}*/
        return $xml; 
    }
	 ...
	 */
	public function postXmlCurl1($xml,$url,$second=30)
	{		//echo $xml."--------".$url."<br>";
	...
	public function postXmlCurl($xml,$url,$second=30,$aHeader=array())
	{
		$ch = curl_init();
		//超时时间
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function setParameter($parameter, $parameterValue)
	{
		...
		//echo $this->parameters[$this->trimString($parameter)]."<br>";
	}
	...
	function createXml()
	{
		//echo "nonce_str----".$this->createNoncestr()."<br>";
