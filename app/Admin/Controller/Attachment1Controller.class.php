...
class Attachment1Controller extends AdminCoreController {
    ...
    protected function _upload_init($upload) {
        ...
        $upload->savePath =  './data/attachment/item/' . $file_type . '/';
		$img_name = md5($_FILES['imgFile']['name']);
        $upload->saveRule = $img_name;
        $upload->autoSub = true;
        $upload->subType = 'date';
		$upload->dateFormat = '0000/00/';
        return $upload;
    }
...
//      $result = $this->_upload($_FILES['imgFile']);
	    ...
	    $result = $this->_upload($_FILES['imgFile'], 'item/'.$date_dir, array(
        ...
        if ($result['error']) {
            ...
            echo json_encode(array('error'=>0, 'url'=>'./data/attachment/item/' . $file_type . '/' . $result['info'][0]['savename']));
        }
        exit;
    }
    ...
    public function editer_manager() {
        $root_path = './data/attachment/item/';
        $root_url = './data/attachment/item/';
