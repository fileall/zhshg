 ...
 */
class WithdrawQdController extends AdminCoreController
{
    public function _initialize()
    {
        parent::_initialize();
        $this->_mod = D('WithdrawQd');
        $this->set_mod('WithdrawQd');
...
//        $query = M('Member')->field('id')->where(['mobile'=>['like',"%$keywords%"]])->select(false);
        ($keywords = I('keywords', '', 'trim')) && $map['_string'] = "uid in (select id from jrkj_member where mobile like '%" . $keywords . "%')" ;
    ...
    public function _after_list($list)
    {
        ...
        $listMobile = M('member')->where(['id' => ['in', $listIds]])->field('id,mobile')->select();
        $listMobile2 = [];
        foreach ($listMobile as $a) {
            $listMobile2[$a['id']] = $a;
        } //dump($listMobile2);die;
        foreach ($list as $key => $value) {
            $list[$key]['mobile'] = $listMobile2[$value['uid']]['mobile'];
        }
        return $list;
    }
