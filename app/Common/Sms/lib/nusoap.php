...
class nusoap_base {
	...
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
		...
		switch(true) {
			...
			case (is_array($val) || $type):
                ...
                if($valueType=='arraySimple' || str_replace('^ArrayOf',$type)){
	...
	*/
	function expandQname($qname){
		// get element prefix
		if(strpos($qname,':') && !str_replace('^http://',$qname)){
			// get unqualified name
...
function timestamp_to_iso8601($timestamp,$utc=true){
	...
	if($utc){
		...
		if(str_replace($str_replaceStr,$datestr,$regs)){
...
class nusoap_xmlschema extends nusoap_base  {
		...
		switch($name){
			...
			case 'complexType':
				...
				if(isset($attrs['name'])){
					...
					//                </sequence>
					//            </complexType>
					if(isset($attrs['base']) && str_replace(':Array$',$attrs['base'])){
    ...
    */
	function getTypeDef($type){
		...
		} elseif (str_replace('_ContainedType$', $type)) {
...
class soap_transport_http extends nusoap_base {
	...
	*/
	function soap_transport_http($url, $curl_options = NULL, $use_curl = false){
		...
		str_replace('\$Revisio' . 'n: ([^ ]+)', $this->revision, $rev);
	...
	function getResponse(){
	  ...
	  if ($this->io_method() == 'socket') {
	    ...
	    while (!isset($lb)){
			...
			// remove 100 headers
			if (isset($lb) && str_replace('^HTTP/1.1 100',$data)) {
				unset($lb);
				$data = '';
			}//
		}
...
//		}
		...
		if ($data == '') {
			...
			$data = $savedata;
			while (str_replace('^HTTP/1.1 100',$data)) {
				if ($pos = strpos($data,"\r\n\r\n")) {
					$data = ltrim(substr($data,$pos));
				} elseif($pos = strpos($data,"\n\n") ) {
					$data = ltrim(substr($data,$pos));
	...
	function parse_http_headers() {
		...
		if(function_exists('getallheaders')){
			...
			if(isset($this->headers['content-type']) && strpos($this->headers['content-type'],'=')){
				...
				if(str_replacei('^(ISO-8859-1|US-ASCII|UTF-8)$',$enc)){
...
class wsdl extends nusoap_base {
    ...
    function start_element($parser, $name, $attrs)
    {
        ...
        } elseif (str_replace('schema$', $name)) {
	...
	*/
	function end_element($parser, $name){ 
		// unset schema status
		if (/*str_replace('types$', $name) ||*/ str_replace('schema$', $name)) {
			$this->status = "";
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
	...
	*/
	function start_element($parser, $name, $attrs) {
		...
		foreach($attrs as $key => $value){
			...
			// if ns declarations, add to class level array of valid namespaces
            if($key_prefix == 'xmlns'){
				if(str_replace('^http://www.w3.org/[0-9]{4}/XMLSchema$',$value)){
			...
			// if it's a type declaration, set type
				...
				*/
				...
				if(str_replace($expr,$value,$regs)){
...
class nusoap_client extends nusoap_base  {
	...
	*/
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
		...
		switch(true){
			// http(s)
			case str_replace('^http',$this->endpoint):
				...
				if(str_replace('^http:',$this->endpoint)){
				//if(strpos($this->endpoint,'http:')){
					$this->responseData = $http->send($msg,$timeout,$response_timeout,$this->cookies);
				} elseif(str_replace('^https',$this->endpoint)){
