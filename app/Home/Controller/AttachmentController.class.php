...
class AttachmentController extends HomeController {
    protected function _upload_init($upload) {
        (!is_login()) && redirect(U('member/login'));
        ...
        $allow_max && $upload->maxSize = $allow_max * 1024;   //文件大小限制 单位字节 byte
        $allow_exts && $upload->allowExts = $allow_exts;  //文件类型限制
        $upload->savePath =  './data/attachment/web/' . $file_type . '/';
    ...
    public function editer_upload() {
        ...
        if ($result['error']) {
            ...
            echo json_encode(array('error'=>0, 'url'=>'/data/attachment/web/' . $file_type . '/' . $result['info'][0]['savename']));
        }
        exit;
    }
    ...
    public function editer_manager() {
        $root_path = './data/attachment/web/';
        $root_url = '/data/attachment/web/';
