...
//	});
	...
	//手机号码匹配
	$(".tel").blur(function(){
        var patter=/^1[3|4|5|8][0-9]\d{4,8}$/;
        if (!patter.test($(this).val())) {
            $(this).val("").attr("placeholder","请输入正确的手机号");
        }
    })
//	//上传预览
...
//	        if (fileupLoad) {
//	        	$(".uploadimg-oneimg").attr('src',''+imgSrc);
//	        }else{
//	           $(this).parent().after("<div class='uploadimg'><img class='w100 uploadimg-oneimg'src='"+imgSrc+"'/></div>");
//	           fileupLoad=true;
//	        }
//	    }else{
//	        for (i in this.files) {
//	        	var imgSrc=getObjectURL(this.files[i]);
//	        	$(this).parent().after("<div class='uploadimg'><img class='w100 uploadimg-img'src='"+imgSrc+"'/></div>");
//	        	fileupLoadNum++;
//	        	if (fileupLoadNum==5) {
//	        		$(this).parent().hide();
//	        	}
...
//	//动态添加监听事件
//	function a(){
//  	$('.y-add-shop-message').on('click','.uploadimg-close',function(){
//          console.log(0)
//          $(this).parent().remove();
//          fileupLoadNum--;
//          if(fileupLoadNum==4){
...
//	}
    ...
    //商家相册
    $(".y-storeAddImg").click(function(){
        var mytext=$(this).text();
        switch (mytext){
        	case '编辑':
        	    $(".y-storeImg-item-remove-bg").show();
        	    $(this).text("保存");
        		break;
        	case '保存':
        	    $(".y-storeImg-item-remove-bg").hide();
        	    $(this).text("编辑");
        	    break;
        	default:
        		break;
        }
        $(".y-storeImg-item-remove-bg").addClass("y-block");
    })
});