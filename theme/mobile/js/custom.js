...
//	});
    ...
    function discuss(e, n) {
        $(e).click(function() {
            var ele =$(this).parent().find('.discuss-star');
            var index = ele.index($(this));
            console.log(ele+"---"+index)
            for(var i = 0; i < ele.length; i++) {
                if(index >= i) {
                    ele.eq(i).addClass("active");
                    ele.eq(i).find("i").addClass("fa-star").removeClass("fa-star-o");
                } else {
                    ele.eq(i).removeClass("active");
                    ele.eq(i).find("i").removeClass("fa-star").addClass("fa-star-o");
                }
            }
        });
    }
    ...
    //密码=>异步请求
        ...
        $(".pay-button.figure").click(function(){
            ...
            if(ONTROL == 1){
                ...
                // var pas = '';
                // var bt = document.getElementsByName("xx");
                //
                // for(var i = 0; i<bt.length; i++){
                //     pas +=  bt[i].value;
                // }
                var pas = PASS_ONE.val() + PASS_TWO.val() + PASS_THREE.val() + PASS_FOUR.val() + PASS_FIVE.val() + PASS_SIX.val();

                var zf_type = $(".pay-wrap > ul > li").find(".pay-input-inner.active").data("pay_type");
                var prices = $('#prives').val();
                var item_type = 1;//订单类型1元宝(余额元宝充值时用..)
                var data='';//json数据

                // 密码框在custom.js的修改》
                // origin_type 调用密码框的网页
                // 1元宝&&金果转好友/输入电话支付=》给商家
                // 2金果回购&&金果寄存&&银楼置换
                // 3订单支付
                // 4扫码支付=》商家
				// 5会员升级
                ...
                if( origin_type== 1){
                    ...
                    var transfer_type = $('#transfer_type').val();//支付币种1元宝2金果
                    data={transfer_type:transfer_type,nums:nums,mobile:mobile,pas:pas};
                }else if(origin_type== 2){
                    var nums = $("#nums").val();
                    data={nums:nums,pas:pas};
                }else if(origin_type== 3){
					data={zftype:zf_type,pas:pas,oid:$('#oid').val()};
				}else if(origin_type== 4){
                    var nums = $("#nums").val();
                    var mobile = $("#mobile").val();
                    var memos = $("#memos").val();
                    var transfer_type= $('.pay-input-inner.active').data('zftype');//支付币种1元宝2金果
                    data={transfer_type:transfer_type,nums:nums,mobile:mobile,memos:memos,pas:pas};
				}else if(origin_type== 5){
                    data={pas:pas,oid:$('#oid').val()};
                }else if(origin_type== 6){
                    var nums = $("#nums").val();
                    var card_id = $('#y-selectCard').attr('data-card_id');
                    var page_type = $('#page_type').val();//请求提现的页面：1会员工资2商家元宝3商家金果4区代工资
                    data={pas:pas,card_id:card_id,nums:nums,page_type:page_type};
                }else if(origin_type== 7){
                    var nums = $("#nums").val();
                    data={pas:pas,count:$('#y-border-top').val()};
                }else if(origin_type== 8){
                    var face_value = $("#face_value").val();
                    var page_num = $("#page_num").val();
                    data={pas:pas,face_value:face_value,page_num:page_num};
                }else{
                    data={zftype:zf_type,prices:prices,item_type:item_type,pas:pas,oid:$('#oid').val()};//原来的界面
				}

                //请求后台
				$.post($('#sealing_ye').val(),data,function(dd){
					if(dd.status==1){
						layer.msg(dd.msg,{icon:1,time:1000},function(){
							location.href = dd.url;
						});
					}else{
						layer.msg(dd.msg,{icon:0,time:1000});
					}
            ...
            // if(zf_type==3&&pas.length==6){
            //
            //     $.post('/index.php?s=/member/recharge_make2.html',{price:prices,zftype:zf_type,item_type:item_type},function(d){
            //
            //         if(d.status ==1){
            //             //余额
            //             $.post('/index.php?s=/member/sealing_ye.html',{zftype:zf_type,prices:prices,dingdan:d.dingdan,item_type:item_type,pas:pas},function(dd){
            //                 if(dd.status==1){
            //                     layer.msg('充值成功',{icon:1,time:1000},function(){
            //                         location.href = "/index.php?s=/member/wallet.html";
            //                     });
            //                 }else{
            //                     layer.msg(dd.msg,{icon:1,time:1000});
            //                 }
            //
            //             },"JSON")
            //
